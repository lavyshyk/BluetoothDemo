...
@Composable
fun FindDevicesScreen(onConnect: (BluetoothDevice) -> Unit) {
    ...
    Column(Modifier.fillMaxSize()) {
        ...
        LazyColumn(
            ...
            if (pairedDevices.isNotEmpty()) {
                stickyHeader{
                    Text(text = "SAVED DEVICES", style = MaterialTheme.typography.titleLarge)
                }
                items(pairedDevices) {
                    BluetoothDeviceItem(
                        bluetoothDevice = it,
            ...
            stickyHeader { Text(text = "NEW DEVICES", style = MaterialTheme.typography.titleLarge) }
            items(devices) { item ->

                BluetoothDeviceItem(
                    bluetoothDevice = item,
                    sampleServerDevices.contains(item),
                    onConnect = onConnect,
                )
            }
...
internal fun BluetoothDeviceItem(
    ...
    Row(
        ...
        Text(text = state)
        Text(text = bluetoothDevice.type.toString())
